@extends('layouts.app')
@section('title', 'Tukar Barang')

@section('content')
<div class="page-header">
    <div>
        <div class="page-title"><i class="fas fa-right-left text-blue"></i> Tukar Barang</div>
        <div class="page-subtitle">
            Atas transaksi <strong>{{ $transaction->invoice_no }}</strong>
            @if($transaction->erp_pos_invoice) ({{ $transaction->erp_pos_invoice }}) @endif
            — {{ local_dt($transaction->created_at, 'd/m/Y H:i') }}
            @if($transaction->customer) &middot; {{ $transaction->customer->name }} @endif
        </div>
    </div>
    <a href="{{ route('transactions.show', $transaction->id) }}" class="btn btn-outline"><i class="fas fa-arrow-left"></i> Kembali</a>
</div>
        ...
        <div class="table-wrap">
            <table>
                ...
                @foreach($transaction->items as $item)
                    @php
                        $max = $returnable[$item->id] ?? 0;
                        $unitValue = $item->quantity > 0 ? $item->subtotal / $item->quantity : 0;
                    @endphp
                    <tr>
                        <td>
                            <div class="font-medium">{{ $item->product_name }}</div>
                            <div class="text-sm text-muted">{{ $item->product_sku }}</div>
                        </td>
                        <td class="money" data-unit="{{ $unitValue }}">Rp {{ number_format($unitValue, 0, ',', '.') }}</td>
                        <td>{{ $max }} / {{ $item->quantity }}</td>
                        <td>
                            @if($max > 0)
                            <input type="number" class="form-control return-qty" style="width:90px"
                                min="0" max="{{ $max }}" value="0"
                                data-item-id="{{ $item->id }}" data-unit="{{ $unitValue }}" oninput="recalc()">
                            @else
                            <span class="badge badge-gray">habis diretur</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
...
let newItems = []; // {product_id, name, sku, price, tax_rate, quantity}
...
function recalc() {
    ...
    document.getElementById('submitBtn').disabled = !(r > 0 && newItems.length > 0 && diff >= 0);
}
...
async function submitExchange() {
    ...
    const returns = [...document.querySelectorAll('.return-qty')]
        .map(el => ({item_id: parseInt(el.dataset.itemId), quantity: parseInt(el.value || 0)}))
    ...
    try {
        const resp = await fetch('{{ route("exchange.store", $transaction->id) }}', {
            method: 'POST',
        ...
        // Cetak struk penjualan pengganti; struk retur dibuka di tab kedua
        window.open(data.print_url, '_blank');
        setTimeout(() => { window.location = '{{ route("transactions.index") }}'; }, 1200);
    } catch (e) {
